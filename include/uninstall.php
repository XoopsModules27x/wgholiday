 ...
 */
function xoops_module_pre_uninstall_wgholiday(\XoopsModule $module)
{
...
function xoops_module_uninstall_wgholiday(\XoopsModule $module)
{
    ...
    if ($dirInfo->isDir()) {
        ...
        if (!rename($uploadDirectory, $uploadDirectory . "_bak_$date")) {
            $module->setErrors(sprintf(constant('_CO_WGHOLIDAY_ERROR_BAD_DEL_PATH'), $uploadDirectory));
            $success = false;
        }
    }
    unset($dirInfo);
    ...
    if (is_file($xmlfile)) {
        if (false === ($delOk = \unlink($xmlfile))) {
            $module->setErrors(\sprintf(\_AM_WGHOLIDAY_ERROR_BAD_REMOVE, $xmlfile));
        }
    }
